...
let {Promise} = Cu.import("resource://gre/modules/Promise.jsm", null);
...
let IO = exports.IO =
{
   ...
   */
  resolveFilePath: function(/**String*/ path, /*optional */ _type) /**nsIFile*/
  {
    if (!path)
      return null;
    ...
    try {
      ...
      res = new FileUtils.File(path);
    } catch (e) {}

    if(!res){
      try {
        // Try relative path now
        res = FileUtils.getFile("ProfD", path.split("/"));
      } catch (e) {}
    }


    if(res && (typeof _type != "undefined")){
      if(!res.exists()){
        res.create(_type, 0o777);
      }
    }
    ...
    try
    {
      ...
      Task.spawn(function*()
      {
        if (firstRead && Services.vc.compare(Utils.platformVersion, "23.0a1") <= 0)
        {
          ...
          try
          {
            let dummyPath = IO.resolveFilePath(settings.data_directory + "/dummy").path;
  ...
  writeToFile: function(/**nsIFile*/ file, /**Iterator*/ data, /**Function*/ callback)
  {
    try
    {
      ...
      Task.spawn(function*()
      {
        ...
        let exists = yield OS.File.exists(file.path);
        if(exists){
